...
fixed_dc=fi(dc,true,16,14);
write_to_file("input_sig_half_KHz.txt",fixed_dc,1);
...
filtered_dc = filter(fir_coefficients, 1,dc);
csvwrite("output_sig_half_KHz.txt",transpose(dc));
...
figure
plot(t*1000,filtered_dc)
xlabel('time (ms)')
ylabel('Amplitude')
title('filtered dc signal')
